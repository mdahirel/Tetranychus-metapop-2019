...
```{r P_formulas}
P_formula <- bf(
  AFEMA ~ (is.landscape8 + is.landscape16) * is.randomized + # we use the dummy centred variables
...
### adapted formula
P_formula2 <- bf(
  AFEMA ~ (is.landscape8 + is.landscape16) * is.randomized +
    (1 | METAPOP_ID / PATCH),
  nlf(shape ~ 1 / exp(loginvshape)),
  loginvshape ~ (is.landscape8 + is.landscape16) * is.randomized +
    (1 | METAPOP_ID / PATCH),
  family = negbinomial(link_shape = "identity")
)
...
P_latent_intercepts <- P_data %>%
  select(
    METAPOP_ID, SHUFFLE, LENGTH, PATCH,
...
```{r obs_summaries}
P_obsmeans <- P_data %>%
  group_by(SHUFFLE, LENGTH, METAPOP_ID, PATCH) %>%
...
save(list=c("pairwise1SHUFFLE","pairwise1LENGTH"),file=here("R_output","pairwise_posteriors_popsize1.Rdata"))
```
...
save(list=c("P_tab_fig2a"),file=here("R_output","pairwise_posteriors_popsize2.Rdata"))
```
