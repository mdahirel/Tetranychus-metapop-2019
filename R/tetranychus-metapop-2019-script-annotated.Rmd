...
P_tab_fig2_length_noshuffle <- P_tab_plots %>%
...
fig2_alpha_a <- ggplot(P_tab_fig2_shuffle) +
...
fig2_alpha_b <- ggplot(P_tab_fig2_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig2_length_noshuffle,
  ...
  # filled version
  stat_halfeye(
    data = P_tab_fig2_length_noshuffle,
    aes(x = mean_alpha, y = LENGTH),
    fill = "grey",
    orientation = "horizontal",
    slab_alpha = 0.3,
...
fig2_phi_a <- ggplot(P_tab_fig2_shuffle) +
...
fig2_phi_b <- ggplot(P_tab_fig2_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig2_length_noshuffle,
...
fig2_gamma_a <- ggplot(P_tab_fig2_shuffle) +
...
fig2_gamma_b <- ggplot(P_tab_fig2_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig2_length_noshuffle,
...
fig2_beta2_a <- ggplot(P_tab_fig2_shuffle) +
...
fig2_beta2_b <- ggplot(P_tab_fig2_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig2_length_noshuffle,
  #             aes(x=mean_uneven,y=LENGTH),
  #             orientation="horizontal",col="grey",slab_linetype=2,slab_size=0.6,
  #             fill=NA,
  #             point_interval=NULL)+
  # filled version
...
((fig2_alpha_a / fig2_alpha_b) |
  (fig2_beta2_a / fig2_beta2_b)|
  (fig2_phi_a / fig2_phi_b) |
  (fig2_gamma_a / fig2_gamma_b) ) &
  cowplot::theme_half_open(11) &
...
pairwise2alpha_shuffle <- P_tab_fig2_shuffle %>%
...
pairwise2phi_shuffle <- P_tab_fig2_shuffle %>%
...
pairwise2gamma_shuffle <- P_tab_fig2_shuffle %>%
...
pairwise2beta2_shuffle <- P_tab_fig2_shuffle %>%
...
pairwise2alpha_length <- P_tab_fig2_length %>%
  compare_levels(
...
pairwise2phi_length <- P_tab_fig2_length %>%
  compare_levels(
...
pairwise2gamma_length <- P_tab_fig2_length %>%
  compare_levels(
...
pairwise2beta2_length <- P_tab_fig2_length %>%
  compare_levels(
...
```{r fig3_tables}
P_tab_fig3a <- P_tab_plots %>%
  group_by(.iteration, LENGTH, SHUFFLE) %>%
  summarise(mean = mean(P_mean_all)) %>%
  ungroup()

P_tab_fig3b <- P_tab_plots %>%
  pivot_longer(
    cols = c(P_mean_center, P_mean_sides, P_mean_corners),
    names_to = "local_connectedness", values_to = "P_mean_local"
  ) %>%
...
# Some stuff to prepare for the revisions/ the supplementary

## let's check if metapops are at equilibrium
...
M_data %>% 
  ...
  ggplot()+
  stat_lineribbon(aes(WEEK,.value),.width=c(0.01,0.95),col="grey")+
  geom_line(data=M_data,
            aes(WEEK,METAPOPSUM,group=METAPOP_ID))+facet_wrap(~METAPOP_ID)
```

# now let's do pairwise comparison plots

see above for now, will update after validated
