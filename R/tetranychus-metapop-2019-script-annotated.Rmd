...
N_chains <- 4
N_warmup <- 1000   ## used for publication: 10000 ## 200 is enough for tests
N_iter <- N_warmup + 1000  ## recommended for publication-level quality: 2000 iterations post warmup when all chains combined 
...
```{r P_plot-local}
P_summary %>% group_by(.iteration,LENGTH, SHUFFLE) %>% 
  summarise(mean_corners = mean(P_mean_corners), 
            mean_sides = mean(P_mean_sides),
            mean_center = mean(P_mean_center)) %>% 
  pivot_longer(mean_corners:mean_center) %>% 
  group_by(.iteration, SHUFFLE, name) %>% 
  summarise(mean=mean(value))%>%
  mutate(local_connectedness = fct_relevel(factor(name) , "mean_corners", after = Inf)) %>% 
  mutate(local_connectedness = fct_recode(local_connectedness,
                                          `center (8 links)` = "mean_center", 
                                          `side (5 links)` = "mean_sides", 
                                          `corner (3 links)`="mean_corners")) %>% 
...
```{r P_alpha}
P_summary %>% group_by(.iteration,LENGTH, SHUFFLE) %>% summarise(mean = mean(P_alpha)) %>% 
  ggplot()+
