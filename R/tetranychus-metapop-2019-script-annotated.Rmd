...
P_tab_fig2_shuffle <- P_tab_plots %>% 
  group_by(.iteration, SHUFFLE) %>%  
  summarise(mean_alpha = mean(P_alpha),
            mean_phi = mean(P_phi),
...
fig2_alpha_b <-  ggplot(P_tab_fig2_length)+
  ...
  #             point_interval=NULL)+
  # filled version
  stat_halfeye(data=P_tab_fig2_length_noshuffle,
               aes(x=mean_alpha,y=LENGTH),
               fill="grey",
               orientation="horizontal",
               slab_alpha=0.3,
...
((fig2_alpha_a/fig2_alpha_b)|
  ...
  cowplot::theme_half_open(11) &
  cowplot::background_grid(colour.major = "grey95", 
                           colour.minor = "grey95") &
  theme(legend.position = "none")
...
P_tab_fig2_length %>% 
  compare_levels(variable=mean_alpha, by=LENGTH,fun=`/`,
                 comparison=list(                  
                   # we give the comparison list explicitly since by default it will give 4cm/16cm
                   # and we'd rather present the reverse
                   c("16 cm (low)","4 cm (high)"),
...
P_tab_fig2_length %>% 
  compare_levels(variable=mean_phi, by=LENGTH,fun=`/`,
                 comparison=list(                  
                   # we give the comparison list explicitly since by default it will give 4cm/16cm
                   # and we'd rather present the reverse
                   c("16 cm (low)","4 cm (high)"),
...
P_tab_fig2_length %>% 
  compare_levels(variable=mean_gamma, by=LENGTH,fun=`/`,
                 comparison=list(                  
                   # we give the comparison list explicitly since by default it will give 4cm/16cm
                   # and we'd rather present the reverse
                   c("16 cm (low)","4 cm (high)"),
...
P_tab_fig2_length %>% 
  compare_levels(variable=mean_uneven, by=LENGTH,fun=`/`,
                 comparison=list(                  
                   # we give the comparison list explicitly since by default it will give 4cm/16cm
                   # and we'd rather present the reverse
                   c("16 cm (low)","4 cm (high)"),
