...
Intuition behind this formulation (latent residuals via OLRE + specify covariance structure) is the same as in this comment by Paul Bürkner (https://github.com/paul-buerkner/brms/issues/600#issuecomment-511677732) 
...
Similarly, we can write a (much simpler) model for the total metapopulation size $M$ (the `METAPOPSUM` column). Because there are no correlations to worry about, we can use a negative binomial model here to model the within-replicate temporal variation:
...
P_obsmeans <- P_data %>%
  ...
  ungroup() %>% 
  mutate(SHUFFLE=fct_recode(as.factor(SHUFFLE), `control`="NO",`randomized`="R")) %>% 
  mutate(LENGTH=fct_recode(as.factor(LENGTH), 
                           `16 cm (low)`="16",
                           `8 cm (medium)`="8",
                           `4 cm (high)`="4")) %>% 
  mutate(local_connectedness=fct_recode(as.factor(local_connectedness), 
                           `corner (low)`="corner (3 links)",
                           `side (medium)`="side (5 links)",
                           `center (high)`="center (8 links)"))
###temp fixes!! need to go back at the start of the code and redo all
```

```{r P_SHUFFLE}
P_shuffle<-P_tab_plots %>% 
  mutate(SHUFFLE=fct_recode(as.factor(SHUFFLE), `control`="NO",`randomized`="R"))
```
...
p1<-P_shuffle %>% 
  ...
  ggplot()+
  geom_jitter(data=P_obsmeans, aes(x=P_mean,y=SHUFFLE,alpha=(1/(P_se/P_mean))),col="grey60",
                     height=0.2,size=3) +
...
p2 <- P_tab_plots %>% 
  ...
  summarise(mean = mean(P_mean_all))  %>% 
  ggplot()+
  geom_jitter(data=P_obsmeans, aes(x=P_mean,y=LENGTH,alpha=(1/(P_se/P_mean))),col="grey60",
                     height=0.2,size=3) +
...
p3 <- P_tab_plots %>% 
  ...
  ggplot()+
  geom_jitter(data=P_obsmeans, aes(x=P_mean,y=local_connectedness,alpha=(1/(P_se/P_mean))),
...
p1<-P_tab_plots %>% 
  ...
  mutate(SHUFFLE=fct_recode(SHUFFLE, control="NO",randomized="R")) %>% 
    mutate(LENGTH=fct_recode(as.factor(LENGTH), 
                           ...
                           `4 cm (high)`="4")) %>% 
  ggplot()+
  geom_jitter(data=P_obsmeans, aes(x=P_mean,y=LENGTH,alpha=(1/(P_se/P_mean))),
              col="grey60",height=0.2,size=3) +
  stat_halfeye(aes(x=mean,y=LENGTH, fill=LENGTH),
               orientation="horizontal",.width=c(0.01,0.95),slab_alpha=0.5)+
  scale_fill_manual(values=paletteLENGTH)+
...
left_join(M_tab,P_tab_M) %>% 
  ...
  geom_point(aes(x=.value_P_trt,y=.value_M_trt, col= SHUFFLE, size=as.numeric(as.character(LENGTH))))+
...
# to develop: preliminary trials of bridge lengths
