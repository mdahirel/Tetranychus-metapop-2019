...
fig2_uneven_a <- ggplot(P_tab_fig2_shuffle) +
  stat_halfeye(aes(x = mean_uneven, y = SHUFFLE, fill = SHUFFLE),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
fig2_uneven_b <- ggplot(P_tab_fig2_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig2_length_noshuffle,
  ...
  # actual posterior
  stat_halfeye(aes(x = mean_uneven, y = LENGTH, fill = LENGTH),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
pairwise2uneven_shuffle <- P_tab_fig2_shuffle %>%
  compare_levels(variable = mean_uneven, by = SHUFFLE, fun = `/`) 

pairwise2uneven_shuffle %>%
  mean_hdi()

pair2uneven_a <- pairwise2uneven_shuffle %>% 
  ggplot() + 
  stat_eye(aes(mean_uneven,SHUFFLE),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("")+
  scale_y_discrete("comparison")
...
pairwise2uneven_length <- P_tab_fig2_length %>%
  compare_levels(
    variable = mean_uneven, by = LENGTH, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give 4cm/16cm
      # and we'd rather present the reverse
      c("16 cm (low)", "4 cm (high)"),
...
pairwise2uneven_length %>%
  mean_hdi()

pair2uneven_b <- pairwise2uneven_length %>% 
  ggplot() + 
  stat_eye(aes(mean_uneven,LENGTH),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
...
(pair2uneven_a/pair2uneven_b) & coord_cartesian(xlim=c(0,4))
```
...
```{r}
M_formula_cont <- bf(
  METAPOPSUM ~ 0 + METAPOP_ID + METAPOP_ID:scale(WEEK),
  nlf(shape ~ 1 / exp(loginvshape)),
  ## we fit the model on the inverse of shape
  ## (avoid assuming a priori that very large overdispersions _ low values of shape _ are more likely)
  ## and we need a log transformation to keep things >0
