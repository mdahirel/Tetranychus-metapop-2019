...
N_chains <- 4
N_warmup <- 10000   ## used for publication: 10000 ## 500 is enough for tests
N_iter <- N_warmup + 2500  ## recommended for publication-level quality: N_warmup + at least 2000  ## Nwarmup + 500 is enough for tests
...
This is implemented in R as follow. First, we need to create a `TREATMENT` variable combining `LENGTH` and `SHUFFLE`.
...
metapopformula <- mvbf(
      ...
      bf(METAPOPSUM ~ 0 + TREATMENT + (1|METAPOP_ID) + (1|gr(WEEK2, by = METAPOP_ID)), family = poisson)
    )
```
...
```{r prior-setting}
  prior <- c(
    ...
    set_prior("normal(4,1)", class = "b", 
              resp = c("METAPOPSUM")),
    set_prior("normal(0,1)", class = "sd", group = "METAPOP_ID", 
              resp = c("P11", "P12", "P13", "P21", "P22", "P23", "P31", "P32", "P33", "METAPOPSUM")),
