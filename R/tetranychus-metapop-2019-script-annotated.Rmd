...
N_chains <- 4
N_warmup <- 100   ## used for publication: 10000 ## 500 is enough for tests
N_iter <- N_warmup + 100  ## recommended for publication-level quality: N_warmup + at least 2000  ## Nwarmup + 500 is enough for tests
...
You can see we are estimating a separate temporal variance-covariance matrix for each replicate $i$. We will average everything in the end, but given the non-linearities everywhere in a GLMM, it is probably best to calculate everything replicate by replicate and only average the final results (we'll come back to these non-linearities later). It is important to note that this also mean this is a model with observation-level random effects [@harrisonUsingObservationlevelRandom2014], so no further overdispersion to worry about.

Similarly, we can write a (simpler) model for the total metapopulation size $M$ (the `METAPPOP_ID` column):
...
```{r prior-setting}
  prior <- c(
    ...
    set_prior("normal(4,1)", class = "b", resp = c("METAPOPSUM")),
    set_prior("normal(0,1)", class = "sd", group = "METAPOP_ID2", resp = c("METAPOPSUM")),
    set_prior("normal(0,1)", class = "sd", group = "WEEK3", resp = c("METAPOPSUM"))
  )
```
...
## these warnings don't pop up when cmdstanr is used as backend (other "normal" warnings will appear with both backends)
...
Note that it can take some time to finish (around 2000-4000 iterations/h/core in my laptop).
