...
N_iter <- N_warmup + 2000 ## recommended for publication-level quality: 2000 iterations post warmup 
## Nwarmup + 200 is probably good enough for tests

```

**Note:** The finished model object for the patch level will likely be large (several 100s of MB, depending on `N_iter`) and take some time to fit (a couple hours to half a day on the few laptops it has been tested on). If this causes problems, we invite you to try a model with a much smaller `N_warmup` and number of iterations post-warmup `N_iter` (see comments in code chunk above). It should be close enough to the posterior distribution reached at convergence, despite warnings, to allow you to get a **general idea** of the results.
...
P_data <- P_data %>%
  ...
  mutate(
    is.local5 = as.numeric(local_connectedness == "side (medium)") - mean(as.numeric(local_connectedness == "side (medium)")),
...
$$
$$
\log(\lambda_{[i,x,y,t]}) = B_{0} + \sum_{j=1}^J({B_{j} \times x_{j[i,x,y]}}) + a_{[i]} + c_{[i,x,y]} + d_{[i,x,y,t]},
$$
$$
a_{[i]} \sim \mathrm{Normal}(0, \sigma_{a}),
$$
$$
c_{[i,x,y]} \sim \mathrm{Normal}(0, \sigma_{c}),
$$
$$
\begin{bmatrix} d_{[i,1,1,t]} \\ ... \\ d_{[i,3,3,t]} \end{bmatrix} 
\sim 
\textrm{MVNormal}
\begin{pmatrix}
...
\boldsymbol{\Omega}_{[i]} = 
\begin{bmatrix}
\sigma_{d[i,1,1]} & 0 & ... \\ 0 & \ddots  \\ \vdots & & \sigma_{d[i,3,3]} 
\end{bmatrix}
\boldsymbol{R}_{[i]}
\begin{bmatrix}
\sigma_{d[i,1,1]} & 0 & ... \\ 0 & \ddots  \\ \vdots & & \sigma_{d[i,3,3]} 
...
where $B_j$ are the fixed effects (with $B_{0}$ the intercept), $a$ are replicate/metapopulation random effects, $c$ patch-level random effects, and $d$ temporal abundance fluctuations (We use Latin letters here, rather than the usual Greek script, to avoid confusion with $\alpha$, $\beta$, $\gamma$ variabilities). $\boldsymbol{\Omega}_{[i]}$ is the temporal covariance matrix for the replicate $i$ and $\boldsymbol{R}_{[i]}$ the corresponding correlation matrix. For implementation, we transform the treatment covariates into dummy centred variables following @schielzethSimpleMeansImprove2010, this has the added benefit of making $B_{0}$ the intercept of the "average" treatment.

We will also fit the "equivalent" negative binomial model, as a check. Intuitively, it does not allow within-patch temporal variation to be correlated across patches, so should lead to inadequate predictions of patch-level variation.
...
$$
$$
\log(\lambda_{[i]}) = B_{0} + \sum_{j=1}^J({B_{j} \times x_{j[i]}}) + c_{[i]},
$$
$$
\log(1/\phi_{[i]}) = a_{0} + \sum_{j=1}^J({a_{j} \times x_{j[i]}}) + d_{[i]},
$$
$$
c_{[i]} \sim \mathrm{Normal}(0, \sigma_{c}),
$$
$$
d_{[i]} \sim \mathrm{Normal}(0, \sigma_{d}).
$$  

Here $c$ and $d$ refer to the metapopulation-level random effects, for the mean parameter and for the shape parameter, respectively. Similarly, $B$ and $a$ refer to the fixed effects coefficients for the mean and the shape. We fit the model for the overdispersion parameter on its log-transformed inverse: the inverse transformation (as suggested in the Stan language wiki: https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations) is used to avoid giving too much prior weight to high overdispersion, the log transformation to keep $\phi$ estimates > 0.
...
) # the transformation link is already "included" in the code just above
...
(This is a copy of the relevant columns in the Dryad data in De Roissart *et al.* [-@deroissartDataSpatialSpatiotemporal2016], see there and data/README.md for details).
...
prior_data <- prior_data %>%
  ...
  ## all the mites on the patch at time t
  group_by(MP, P, Treatment) %>%
  ## metapop plus patch ID uniquely define a patch
  mutate(was_populated = mean(total) > 0) %>%
  ## to exclude all patches that were NEVER populated
  ungroup() %>%
...
For the random effect SDs, one can use the total log-scale variance in prior data as a guide, so roughly 1. The "typical" sum of replicate level variance, patch-level variance and within-patch level variance should then be around 0.9-1. So the typical prior variance for each of the 3 components should be around 0.3, leading to prior SDs around 0.5-0.55 as a pretty typical value.

(note: specifying larger priors is not dramatic and does not change outcome of among-treatment comparisons of means, but tends to shift all predicted means on the observed scale a bit higher than what is actually observed.)
...
if (file.exists(here("R_output", "P_mod.Rdata"))) {
  ...
  P_mod <- brm(
    formula = P_formula, prior = P_prior, data = P_data,
...
if (file.exists(here("R_output", "M_mod.Rdata"))) {
  ...
  M_mod <- brm(
    formula = M_formula, prior = M_prior, data = M_data,
...
## for P_mod, basic pp_checks will return good-to-perfect fits, since there are obs-level random effects
```
...
if (file.exists(here("R_output", "P_mod_alt.Rdata"))) {
  ...
  P_mod2 <- brm(
    formula = P_formula2, prior = P_prior2, data = P_data,
    iter = N_iter, warmup = N_warmup, chains = N_chains,
    seed = 42, control = list(adapt_delta = 0.9, max_treedepth = 15),
    backend = "cmdstanr"
  )
...
```{r extract_P_vcv}
memory.limit(40000) #### if needed, a little memory allowance boost above defaults to handle the full vcv matrices in final model
...
P_latent_intercepts <- P_data %>%
  select(
    METAPOP_ID, SHUFFLE, LENGTH, local_connectedness, PATCH,
...
P_latent_intercepts <- P_latent_intercepts %>%
  ...
  mutate(P_latent_intercept = map(
    .x = P_latent_intercept,
    .f = ~ .x %>% unlist()
  ))
```
...
```{r P_variabilities}
P_tab <- P_tab %>%
  mutate(P_psi = map(.x = P_pred, .f = function(.x) {
    diag(.x)
  })) %>%
  mutate(P_vcv_time_obs = map2(
    .x = P_psi, .y = P_vcv_time_latent,
    .f = ~ (.x %*% .y %*% t(.x))
  )) %>%
  mutate(
    P_alpha = map2(.x = P_vcv_time_obs, .y = P_pred, 
                   .f = ~ .x %>% alpha_wang_loreau(varcorr = ., means = .y)),
    P_gamma = map2(.x = P_vcv_time_obs, .y = P_pred, 
                   .f = ~ .x %>% gamma_wang_loreau(varcorr = ., means = .y)),
    P_uneven = map2(.x = P_vcv_time_obs, .y = P_pred, 
                    .f = ~ .x %>% uneven_wang_loreau(varcorr = ., means = .y))
  ) %>%
  unnest(cols = c(P_alpha, P_gamma, P_uneven)) %>%
  mutate(
    P_beta1 = P_alpha / P_gamma, 
    P_phi = P_gamma / P_alpha, 
    P_beta2 = P_alpha - P_gamma
  )
```
...
P_tab_plots <- P_tab %>%
  ungroup() %>%
  select(-c(
    P_vcv_time_latent, P_latent_intercept,
    P_pred, P_psi, P_vcv_time_obs
  ))
...
# Part 4- Figures and comparisons
...
```{r figure_palettes}
paletteLENGTH <- c("#D55E00", "#E69F00", "#F0E442")
paletteLOCAL <- c("#e66101", "#fdb863", "#5e3c99")
```
...
```{r fig1}
fig1a <- ggplot(P_tab_fig1a) +
  geom_jitter(
    data = P_obsmeans,
    aes(x = P_mean_obs, y = SHUFFLE, alpha = 1 / (P_var_obs / P_mean_obs^2)),
...
fig1b <- ggplot(P_tab_fig1b) +
  geom_jitter(
    data = P_obsmeans,
    aes(x = P_mean_obs, y = LENGTH, alpha = 1 / (P_var_obs / P_mean_obs^2)),
    col = "grey60", height = 0.2, size = 3
  ) +
...
fig1c <- ggplot(P_tab_fig1c) +
  geom_jitter(
    data = P_obsmeans,
    aes(
      x = P_mean_obs, y = local_connectedness,
      alpha = 1 / (P_var_obs / P_mean_obs^2)
    ),
    col = "grey60", height = 0.2, size = 3
  ) +
...
pairwise1SHUFFLE <- P_tab_fig1a %>%
  compare_levels(variable = mean, by = SHUFFLE, fun = `/`) 

pairwise1SHUFFLE %>%
  mean_hdi()

pair1a <- pairwise1SHUFFLE %>% 
  ggplot() + 
  stat_eye(aes(mean,SHUFFLE),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
...
pairwise1LENGTH <- P_tab_fig1b %>%
  compare_levels(
    variable = mean, by = LENGTH, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give 4cm/16cm
      # and we'd rather present the reverse
      c("16 cm (low)", "4 cm (high)"),
      c("16 cm (low)", "8 cm (medium)"),
      c("8 cm (medium)", "4 cm (high)")
    )
  ) 
...
pair1b <- pairwise1LENGTH %>% 
  ggplot() + 
  stat_eye(aes(mean,LENGTH),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("")+
...
pairwise1local <- P_tab_fig1c %>%
  compare_levels(
    variable = mean, by = local_connectedness, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give center/corner
      # and we'd rather present the reverse
      c("corner (low)", "center (high)"),
      c("corner (low)", "side (medium)"),
      c("side (medium)", "center (high)")
    )
  ) 
...
pair1c <- pairwise1local %>% 
  ggplot() + 
  stat_eye(aes(mean,local_connectedness),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("posterior ratio of mean population sizes")+
  scale_y_discrete("")

(pair1a/pair1b/pair1c) &
  coord_cartesian(xlim=c(0.75,1.35))

save(list=c("pairwise1SHUFFLE","pairwise1LENGTH","pairwise1local"),file=here("R_output","pairwise_posteriors_popsize1.Rdata"))
```
...
We here basically the same as in Figure 1, but splitting randomized and non-randomized replicates instead of pooling them together:

```{r fig2_tables}
P_tab_fig2a <- P_tab_plots %>%
  group_by(.iteration, LENGTH, SHUFFLE) %>%
  summarise(mean = mean(P_mean_all)) %>%
  ungroup()

P_tab_fig2b <- P_tab_plots %>%
  pivot_longer(
    cols = c(P_mean_center, P_mean_sides, P_mean_corners),
    names_to = "local_connectedness", values_to = "P_mean_local"
  ) %>%
...
```{r fig2}
fig2a <- ggplot(P_tab_fig2a) +
  geom_jitter(
    data = P_obsmeans,
    aes(
      x = P_mean_obs, y = LENGTH,
      alpha = 1 / (P_var_obs / P_mean_obs^2)
    ),
    col = "grey60", height = 0.2, size = 3
  ) +
...
# another version using local connectedness, not used in present version of manuscript
fig2b <- ggplot(P_tab_fig2b) +
  geom_jitter(
    data = P_obsmeans,
    aes(
      x = P_mean_obs, y = local_connectedness,
...
## we estimate posterior for local and metapop connectedness,but only display comparisons for 
## metapop in main figure (see supplementary for all comparisons)
...
save(list=c("P_tab_fig2a","P_tab_fig2b"),file=here("R_output","pairwise_posteriors_popsize2.Rdata"))
```

### Pairwise comparisons

```{r pairwise_comp_fig2}
P_tab_fig2a %>%
  group_by(SHUFFLE) %>%
  compare_levels(
    variable = mean, by = LENGTH, fun = `/`,
...
P_tab_fig2a %>% 
  mutate(group=paste(LENGTH, SHUFFLE)) %>% 
  compare_levels(variable = mean, by = group, fun = `/`) %>%
  ggplot() + 
  stat_eye(aes(mean,group),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("posterior ratio of mean population sizes")+
  scale_y_discrete("comparison")
...
P_tab_fig2b %>%
  group_by(local_connectedness) %>%
  compare_levels(variable = mean, by = SHUFFLE, fun = `/`) %>%
  mean_hdi()

P_tab_fig2b %>% 
  mutate(group=paste(local_connectedness, SHUFFLE)) %>% 
  compare_levels(variable = mean, by = group, fun = `/`) %>%
  ggplot() + 
  stat_eye(aes(mean,group),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
...
### Figure 3: uses simulation outputs

Here we collect the result from the simulation, and use them to display how variability metric vary with the level of connectedness (here modelled as dispersal mortality)
...
B1 <- ggplot(sim_mortality, aes(x = disp_mort, y = betavar)) +
  geom_point(size = 2, col = "grey80") +
  geom_smooth(method = "gam", col = "black") +
  scale_y_continuous(expression(paste("Additive spatial variability   ", beta[2])),
    lim = c(0.005, 0.025)
  ) +
  scale_x_continuous("Dispersal cost (mortality)")

C1 <- ggplot(sim_mortality, aes(x = disp_mort, y = 1 / betavar_)) +
  geom_point(size = 2, col = "grey80") +
  geom_smooth(method = "gam", col = "black") +
  scale_y_continuous(expression(paste("Spatial synchrony  ", phi1)),
    lim = c(0.05, 0.25)
  ) +
  scale_x_continuous("Dispersal cost (mortality)")

D1 <- ggplot(sim_mortality, aes(x = disp_mort, y = gammavar)) +
  geom_point(size = 2, col = "grey80") +
  geom_smooth(method = "gam", col = "black") +
...
For the final figure, we display the posterior for the average variability metrics for each metapopulation-level treatment (randomized versus control on one side, low vs. medium vs. high metapopulation-connectedness on the other). In the latter, we additionnally add (as a pale "shadow") what would be the posteriors if we only used control replicates in the averaging. 

```{r fig4_tables}
P_tab_fig4_shuffle <- P_tab_plots %>%
  group_by(.iteration, SHUFFLE) %>%
  summarise(
...
## for the connectedness effect, we do the same table without the shuffled replicates
## this allows us to display in grey the "ghost" of the posterior we would have
## if we excluded shuffled replicates
## showing our results are mostly independent of shuffling
P_tab_fig4_length_noshuffle <- P_tab_plots %>%
  filter(SHUFFLE == "control") %>%
  group_by(.iteration, LENGTH) %>%
  summarise(
    mean_alpha = mean(P_alpha),
    mean_phi = mean(P_phi),
...
fig4_alpha_a <- ggplot(P_tab_fig4_shuffle) +
  stat_halfeye(aes(x = mean_alpha, y = SHUFFLE, fill = SHUFFLE),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
fig4_alpha_b <- ggplot(P_tab_fig4_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig4_length_noshuffle,
  #             aes(x=mean_alpha,y=LENGTH),
  #             orientation="horizontal",col="grey",slab_linetype=2,slab_size=0.6,
  #             fill=NA,
  #             point_interval=NULL)+
  # filled version
  stat_halfeye(
    data = P_tab_fig4_length_noshuffle,
    aes(x = mean_alpha, y = LENGTH),
    fill = "grey",
    orientation = "horizontal",
    slab_alpha = 0.3,
  ...
  # actual posterior
  stat_halfeye(aes(x = mean_alpha, y = LENGTH, fill = LENGTH),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
fig4_phi_a <- ggplot(P_tab_fig4_shuffle) +
  stat_halfeye(aes(x = mean_phi, y = SHUFFLE, fill = SHUFFLE),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
fig4_phi_b <- ggplot(P_tab_fig4_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig4_length_noshuffle,
  #             aes(x=mean_phi,y=LENGTH),
  #             orientation="horizontal",col="grey",slab_linetype=2,slab_size=0.6,
  #             fill=NA,
  #             point_interval=NULL)+
  # filled version
  ...
  # actual posterior
  stat_halfeye(aes(x = mean_phi, y = LENGTH, fill = LENGTH),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
  scale_fill_manual(values = paletteLENGTH) +
  scale_x_continuous(expression(paste("mean spatial synchrony ", phi1))) + # phi1 to get the cursive
  scale_y_discrete("") +
  coord_cartesian(xlim = c(0.20, 0.35))

fig4_gamma_a <- ggplot(P_tab_fig4_shuffle) +
  stat_halfeye(aes(x = mean_gamma, y = SHUFFLE, fill = SHUFFLE),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
fig4_gamma_b <- ggplot(P_tab_fig4_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig4_length_noshuffle,
  #             aes(x=mean_gamma,y=LENGTH),
  #             orientation="horizontal",col="grey",slab_linetype=2,slab_size=0.6,
  #             fill=NA,
  #             point_interval=NULL)+
  # filled version
  ...
  # actual posterior
  stat_halfeye(aes(x = mean_gamma, y = LENGTH, fill = LENGTH),
    orientation = "horizontal",
    .width = c(0.01, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
...
fig4_beta2_a <- ggplot(P_tab_fig4_shuffle) +
...
fig4_beta2_b <- ggplot(P_tab_fig4_length) +
  ## line version of the "ghost"
  # stat_slab(data=P_tab_fig4_length_noshuffle,
  #             aes(x=mean_beta2,y=LENGTH),
  #             orientation="horizontal",col="grey",slab_linetype=2,slab_size=0.6,
  #             fill=NA,
  #             point_interval=NULL)+
  # filled version
  ...
  scale_fill_manual(values = paletteLENGTH) +
  scale_x_continuous(expression(paste("mean additive spatial variability ", beta[2]))) +
  scale_y_discrete("") +
  coord_cartesian(xlim = c(0.5, 1))

((fig4_alpha_a / fig4_alpha_b) |
  (fig4_beta2_a / fig4_beta2_b)|
  (fig4_phi_a / fig4_phi_b) |
  (fig4_gamma_a / fig4_gamma_b) ) &
  cowplot::theme_half_open(11) &
...
```{r pairwise_comp_fig4}
pairwise2alpha_shuffle <- P_tab_fig4_shuffle %>%
  compare_levels(variable = mean_alpha, by = SHUFFLE, fun = `/`) 

pairwise2alpha_shuffle %>%
  mean_hdi()

pair2alpha_a <- pairwise2alpha_shuffle %>% 
  ggplot() + 
  stat_eye(aes(mean_alpha,SHUFFLE),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("")+
  scale_y_discrete("comparison")

pairwise2phi_shuffle <- P_tab_fig4_shuffle %>%
  compare_levels(variable = mean_phi, by = SHUFFLE, fun = `/`) 

pairwise2phi_shuffle %>%
  mean_hdi()

pair2phi_a <- pairwise2phi_shuffle %>% 
  ggplot() + 
  stat_eye(aes(mean_phi,SHUFFLE),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("")+
  scale_y_discrete("comparison")

pairwise2gamma_shuffle <- P_tab_fig4_shuffle %>%
  compare_levels(variable = mean_gamma, by = SHUFFLE, fun = `/`) 

pairwise2gamma_shuffle %>%
  mean_hdi()

pair2gamma_a <- pairwise2gamma_shuffle %>% 
  ggplot() + 
  stat_eye(aes(mean_gamma,SHUFFLE),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("")+
  scale_y_discrete("comparison")

pairwise2beta2_shuffle <- P_tab_fig4_shuffle %>%
  compare_levels(variable = mean_beta2, by = SHUFFLE, fun = `/`) 

pairwise2beta2_shuffle %>%
  mean_hdi()

pair2beta2_a <- pairwise2beta2_shuffle %>% 
  ggplot() + 
  stat_eye(aes(mean_beta2,SHUFFLE),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("")+
  scale_y_discrete("comparison")

pairwise2alpha_length <- P_tab_fig4_length %>%
  compare_levels(
    variable = mean_alpha, by = LENGTH, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give 4cm/16cm
      # and we'd rather present the reverse
      c("16 cm (low)", "4 cm (high)"),
...
pairwise2alpha_length %>%
  mean_hdi()

pair2alpha_b <- pairwise2alpha_length %>% 
  ggplot() + 
  stat_eye(aes(mean_alpha,LENGTH),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("posterior ratio of mean alphas")+
  scale_y_discrete("comparison")

pairwise2phi_length <- P_tab_fig4_length %>%
  compare_levels(
    variable = mean_phi, by = LENGTH, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give 4cm/16cm
      # and we'd rather present the reverse
      c("16 cm (low)", "4 cm (high)"),
...
pair2phi_b <- pairwise2phi_length %>% 
  ggplot() + 
  stat_eye(aes(mean_phi,LENGTH),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("posterior ratio of mean phis")+
  scale_y_discrete("comparison")

pairwise2gamma_length <- P_tab_fig4_length %>%
  compare_levels(
    variable = mean_gamma, by = LENGTH, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give 4cm/16cm
      # and we'd rather present the reverse
      c("16 cm (low)", "4 cm (high)"),
...
pairwise2gamma_length %>%
  mean_hdi()

pair2gamma_b <- pairwise2gamma_length %>% 
  ggplot() + 
  stat_eye(aes(mean_gamma,LENGTH),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("posterior ratio of mean gammas")+
  scale_y_discrete("comparison")

pairwise2beta2_length <- P_tab_fig4_length %>%
  compare_levels(
    variable = mean_beta2, by = LENGTH, fun = `/`,
    comparison = list(
      # we give the comparison list explicitly since by default it will give 4cm/16cm
      # and we'd rather present the reverse
      c("16 cm (low)", "4 cm (high)"),
...
pairwise2beta2_length %>%
  mean_hdi()

pair2beta2_b <- pairwise2beta2_length %>% 
  ggplot() + 
  stat_eye(aes(mean_beta2,LENGTH),.width=c(0.01,0.95),
           point_interval=mean_hdi)+
  geom_vline(xintercept = 1)+
  scale_x_continuous("posterior ratio of mean unevens")+
  scale_y_discrete("comparison")


(pair2alpha_a/pair2alpha_b) & coord_cartesian(xlim=c(0.6,1.4))
(pair2phi_a/pair2phi_b) & coord_cartesian(xlim=c(0.6,1.4))
(pair2gamma_a/pair2gamma_b) & coord_cartesian(xlim=c(0.6,1.5))
(pair2beta2_a/pair2beta2_b) & coord_cartesian(xlim=c(0.5,1.5))

save(list=c("pairwise2alpha_length",
            "pairwise2beta2_length",
            "pairwise2phi_length",
            "pairwise2gamma_length",
            "pairwise2alpha_shuffle",
            ...
            "pairwise2phi_shuffle",
            "pairwise2gamma_shuffle"),
     file=here("R_output","pairwise_posteriors_variabilities.Rdata"))
```
