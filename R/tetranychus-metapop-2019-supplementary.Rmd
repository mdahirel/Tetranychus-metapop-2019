...
$$
$$
\log(\lambda_{[i,x,y,t]}) = \beta_{0} + \sum_{j=1}^J{\beta_{j}x_{i,j,x,y}} + \alpha_{[i]} + \gamma_{[i,x,y]} + \eta_{[i,x,y,t]},
$$
$$
...
$$
$$
\log(\lambda_{[i]}) = \beta_{0} + \sum_{j=1}^J{\beta_{j}x_{i,j}} + \gamma_{[i]},
$$
$$
...
Given the models complexity, we combined general weakly informative priors sensu McElreath [-@mcelreath2020] with more informative priors based on preexisting density data from a previous study [@deroissartDataSpatialSpatiotemporal2016; @deroissartSpatialSpatiotemporalVariation2015], which we multiplied as needed to match the total area of a patch/a metapopulation. Ignoring these prior sources of information led to models predicting consistently too high abundances (not shown), although relative differences between treatments remained qualitatively similar.
...
## ODD --  Individual heterogeneity and its importance for metapopulation dynamics
...
**Reproduction** -- Each individual reproduces resulting in a number of offspring ($f$) which is drawn from a Poisson distribution:

$$ f \sim \mathrm{Poisson}(\mu_{f}).$$
...
$$\mu_{f}= r \begin{pmatrix}\frac{1+N(r-1)}{a}\end{pmatrix}^b,$$
...
```{r displaying-s5-plot, fig.width=5, fig.height=6.8}
...
**Figure S05.1** -- Effect of dispersal cost (mortality) on key metapopulation metrics in simulations
