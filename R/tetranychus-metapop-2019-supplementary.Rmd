...
authors: Stefano masier, Maxime Dahirel, Frederik Mortier, Dries Bonte
output: 
  html_document:
    theme: yeti
    toc: TRUE
...
where $\beta$ are the fixed effects (with $\beta_{0}$ the intercept), $\alpha$ are replicate/metapopulation random effects, $\gamma$ patch-level random effects, and $\eta$ temporal abundance fluctuations (not to be confused with $\alpha$, $\beta$, $\gamma$ variabilities). $\boldsymbol{\Omega}_{[i]}$ is the temporal covariance matrix for the replicate $i$ and $\boldsymbol{R}_{[i]}$ the corresponding correlation matrix. For implementation, we transform the treatment covariates into dummy centred variables following , this has the added benefit of making $\beta_{0}$ the intercept of the "average" treatment.
...
Given the models complexity, we combined general weakly informative priors sensu McElreath [-@mcelreath2020] with more informative priors based on preexisting abundance data [@deroissartDataSpatialSpatiotemporal2016; @deroissartSpatialSpatiotemporalVariation2015]. Ignoring these prior sources of information led to models predicting consistently too high abundances (not shown), although relative differences between treatments remained qualitatively similar.
...
In the metapopulation-level model, we replaced the prior for $\beta_{0}$ by $\mathrm{Normal}(5,1)$ in order to match the fact that metapopulations contain 9 patches, and used general purpose $\mathrm{Normal}(0,1)$ for all parameters linked to the shape $\phi$.
...
P_tab_M_trt <- P_tab_plots %>% 
  select(LENGTH,SHUFFLE,METAPOP_ID, P_mean_all,.iteration) %>% 
...
left_join(M_tab,P_tab_M) %>% 
  ...
  geom_point(aes(x=.value_P_trt,y=.value_M_trt, col= SHUFFLE, size=as.numeric(factor(LENGTH))))+
...
THIS IS A DRAFT FIGURE, LOTS OF THINGS TO FINE TUNE

S05 -- TO COMPLETE -- something about the simulation


```{r import-simulation-output}
sim_output <- read_csv(here("simulations_output","tetranychus-metapop-2019-simulations-output.csv"))
...
```{r test-plot}
ggplot(sim_output)+
  geom_point(aes(x=disp_prop,y=betavar_),size=2,col="grey80")+
  geom_smooth(aes(x=disp_prop,y=betavar_),method="lm")+
  scale_y_continuous("beta variability (multiplicative)")+
  scale_x_continuous("dispersal variable (which one??)") +
  cowplot::theme_half_open(11) +
  cowplot::background_grid(colour.major = "grey95", colour.minor = "grey95")
```
