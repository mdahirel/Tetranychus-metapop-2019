---
title: "Analysis of replicated Tetranychus metapopulation experiment - abundance and temporal variability"
...
This script relies heavily on REF (for converting latent-scale variance-covariance matrices to observed scale) and REF (for the principle and formula of alpha, beta, gamma variability in metapops). Everything below assumes the user has read these two papers.

# Part 1 : Preparation

This script assumes the "tetranychus-metapop-2019-dataset.csv" dataset is loaded under the name data, using e.g. something like that
...
First, we’re going to load all the packages we need. This includes the tidyverse packages, for data wrangling at the beginning and plotting at the end, as well as the packages needed to do Bayesian inference and post-processing of Bayesian model results.
This script uses the Stan language for model fitting.  The RStan package containing Stan has a non-standard installation process, please check online: https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started . The brms package (REF) allows you to write a wide range of Stan models using R syntax; it then does the “translation” before fitting. The QGglmm package....
...
We’re going to set some Stan options, and create variables to create some others for later. These can be adjusted depending on models. More chains (Nchains)/longer (Niter) chains than the default can be useful to ensure convergence.  mc.cores tells R how many cores Stan can use simultaneously. You can fit as many chains simultaneously as there are cores available. If Nchains > mc.cores, they will simply be fitted sequentially. The setup below uses all the chains; this can make it difficult to use a laptop for other tasks.
...
In the raw dataset, data are stored so that there is one column for all population sizes, and a column saying which patch (coded by row and column) it belongs to. To fit our multivariate model, we need one column per patch position:
...
One of the aims of this script is estimate the alpha, beta and gamma variability metrics proposed by REF, while accounting for among metapopulation-variability. I’ve written short functions that calculate these metrics, given as input a variance-covariance matrix, and a vector of mean population sizes (both must be numeric and contain the same number of patches, of course).
...
We fit a multivariate generalized linear model to the abundance data, with one submodel for each of the 9 patches. This Poisson model (because count data) includes "random" effects for each metapopulation replicates (to account for the fact some replicates may have higher/lower average population sizes). It also includes a time random effect, to account for temporal patch variance-covariance. This is this second effect we are most interested in here.
The basic formula for the model is as follow:

[latex formula to add later]

(you can see we are estimating a separate temporal variance-covariance matrix for each replicate. We will average everything in this end, but given the non-linearities everywhere, it is probably best to calculate everything replicate by replicate and only average the final results)
...
```{r treatment_select}
which.trt=c("SHUFFLE")  ###you say HERE what treatment you want, if you want an interaction, just put c("FACTOR1","FACTOR2")
##if you want the model with everything pooled, keep the NA
##it converts it as a factor automatically
shared.among.metapop.var= FALSE ##if common among metapop variance
```
...
(note the ~0 + TREATMENT, rather than ~TREATMENT syntax. It’s just a writing trick to directly obtain the coefficients we want and reduce the quantity of post-processing calculations needed. See REF schielzelth and mcelreath (the latter calls it “indexing” or something like that.


We then select our prior. The priors I propose here are weakly informative and based on REF. For the fixed effects coefficients (corresponding to latent grand means of single patch population size), I based myself on previous papers

```{r prior_setting}

if(is.na(which.trt)==TRUE){
prior <- c(
  set_prior("normal(5,0.5)", class = "Intercept", resp = c("P11", "P12", "P13", "P21", "P22", "P23", "P31", "P32", "P33")),
...
if(is.na(which.trt)==FALSE){
  prior <- c(
    set_prior("normal(5,0.5)", class = "b", resp = c("P11", "P12", "P13", "P21", "P22", "P23", "P31", "P32", "P33")),
    set_prior("exponential(1)", class = "sd", resp = c("P11", "P12", "P13", "P21", "P22", "P23", "P31", "P32", "P33")),
    set_prior("lkj(2)", class = "cor") # midly skeptical of very high correlations (positive or negative)
  )
}
...
```{r model_fitting}
mod <- brm(metapopformula,data = tab, iter = Niter, warmup = Nwarmup, chains=Nchains, prior = prior, seed=42)
```

Note that it can take some time to finish (around 1h per 2000 iterations for a 4-chain model in my laptop).

# Part3-Extracting key metrics from model output

A big function that take a model as input and give all metrics as output


```{r metrics_calculation_function}
###need a rewrite to optimise it with apply
...
for (h in 1:length(METAPOPlevels)){
 ...
 if(is.na(which.trt)==TRUE){ 
  latent_intercept <- as.data.frame(fixef(mod, summary = FALSE)) + 
    ranef(mod,summary=FALSE)$METAPOP_ID[,METAPOPlevels[h],] ### intercept means on the latent scale, excluding time effect
 }
...
tab_results<-bind_rows(tab_results) %>% 
  mutate(deltaN_corners_center=Nmean_corners-Nmean_center,
         deltaN_sides_center=Nmean_sides-Nmean_center,
         deltaN_corners_sides=Nmean_corners-Nmean_sides) %>% 
  group_by(TREATMENT,which_model,.iteration) %>% summarize_if(is.numeric, mean)  #alt: list(mean,var) 

return(tab_results)
}
```

once the function is written, to get what we want, we just have to
```{r get_results}
tab_results<-metrics_estimate(mod) ##change model name as appropriate here
```
(Note: it it will take time to go through the loop)

# Part 4- Summarizing the output and inference

```{r summary_inference_plot}
ggplot(tab_results)+geom_density(aes(beta1,col=TREATMENT))

##calculating differences
#
compare_levels(tab_results,variable="gamma",by=TREATMENT) %>%
  ggplot() +
  geom_halfeyeh(aes(y= TREATMENT, x = gamma))+
