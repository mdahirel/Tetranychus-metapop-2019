...
One of the aims of this script is estimate the alpha, beta and gamma variability metrics proposed by Wang and Loreau (2014)[-@wang_ecosystem_2014], while accounting for among metapopulation-variability. I’ve written short functions that calculate these metrics, given as input a variance-covariance matrix, and a vector of mean population sizes (both must be numeric and contain the same number of patches, of course).
...
(you can see we are estimating a separate temporal variance-covariance matrix for each replicate. We will average everything in this end, but given the non-linearities everywhere in a GLMM, it is probably best to calculate everything replicate by replicate and only average the final results)
...
```{r treatment_select}
which.trt=c("SHUFFLE")  ## CHANGE as needed
...
(note the ~0 + TREATMENT, rather than ~TREATMENT syntax in the fixed effects. It’s just a writing trick to directly obtain the coefficients we want and reduce the quantity of post-processing calculations needed. See REF schielzelth and mcelreath (the latter calls it “indexing” or something like that.
...
(of course you can change the model name if you fit several models to compare in the same session)
...
Once, we have fitted and chosen a model, it's time to extract what we need from it. I've made a function a brms model as input and give all metrics we need as output (it only works on one of the models that can be specified via the script above, of course)

In some places in the function you will see 3-dimensional arrays called. This is going to be the typical structure of many parts of a multivariate object:
...
```
(Note: it will take time to go through the loop, I need to optimise the function)
